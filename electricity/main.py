...
parser.add_argument('--nhid', type=int, default=321)
...
log_dir = f"logs/electricity_tcn_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
os.makedirs(log_dir, exist_ok=True)

file_path = os.path.join(os.path.dirname(__file__), 'data', 'electricity.csv')
...
model = PatchTreeTCN(
    input_channels, input_channels, channel_sizes,
...
def train():
    ...
    with open(log_file, 'w') as f:
        f.write(f"TCN Electricity Model Training Log\n")
